...
<aside class="main-sidebar">
    <section class="sidebar">
        ...
        <ul class="sidebar-menu" data-widget="tree">
            ...
            <li><a href="/admin/inventory/suppliers"><i class="fa fa-circle-o"></i><span>Supplier</span></a></li>
                    ...
                    <table class="table table-bordered content-table table-hover table-responsive">
                        <thead>
                            <tr>
                                ...
                                <th class="hidden-xs">Unit Measure</th>
                                <th class="hidden-xs">Active</th>
                                <th class="hidden-xs">Product Type</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><input type="checkbox" class="checker" data-id="{{$product->id}}"></td>
                                <td class="hidden-xs">{{$product->name}}</td>
                                <td class="hidden-xs">{{$product->sale_price}}</td>
                                <td class="hidden-xs">{{$product->cost_price}}</a></td>
                                <td> {{$product->unit_measure->name}}</td>
                                <td class="hidden-xs">{{$product->is_active}}</a></td>
                                <td class="hidden-xs">{{$product->product_type}}</td>
                                 <td class="hidden-xs">{{$product->quantity}}</td>
                                  <td class="hidden-xs">{{$product->barcode}}</td>
                                   {{--  <input type="button" class="btn bg-red btn-flat delete" value="Delete"> --}}
                               {{--  <form action="{{ route('delete.product', $product->id) }}" method="post" id="delete">
                                    {!! method_field('delete') !!}
                                    @csrf
                                </form> --}}
    ...
    $('.action-delete').click(function(){
        ...
        if(confirmation == true){
            $('.checker.ready').each(function(){
                var id = $(this).data('id');
                $.ajax({
                    url: '/admin/inventory/products/ajax/'+id,
                    type: 'delete',
